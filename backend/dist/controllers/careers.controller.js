...
exports.getCareerStats = exports.getCareerById = exports.getAllCareers = void 0;
...
const getAllCareers = async (req, res) => {
    try {
        const { search, category, experienceLevel, minSalary, maxSalary, stressLevel, growthPotential, limit = 50, offset = 0, } = req.query;
        ...
        if (minSalary || maxSalary) {
            filteredCareers = filteredCareers.filter(career => {
                ...
                const salaryParts = career.avgSalaryVND.split('-').map(s => parseInt(s.trim().replace(/,/g, '')));
                if (salaryParts.length !== 2)
                    return false;
                const [careerMinSalary, careerMaxSalary] = salaryParts;
                // Check if career salary range overlaps with filter range
                if (minSalary && careerMaxSalary < Number(minSalary))
                    return false;
                if (maxSalary && careerMinSalary > Number(maxSalary))
                    return false;
                return true;
            });
        }
        ...
        const transformedCareers = filteredCareers.map(career => {
            ...
            const inferExperienceLevel = () => {
                ...
                if (name.includes('intern'))
                    return 'Intern';
                if (name.includes('vp') || name.includes('cmo') || name.includes('chief'))
                    return 'Executive';
                if (name.includes('senior') || name.includes('director') || name.includes('head of'))
                    return 'Senior';
                if (name.includes('manager') || name.includes('lead'))
                    return 'Mid-Level';
                if (name.includes('junior') || name.includes('coordinator') || name.includes('specialist'))
                    return 'Entry';
                return 'Mid-Level';
            };
            return {
...
const getCareerStats = async (req, res) => {
    try {
        ...
        const byCategory = allCareers.reduce((acc, career) => {
            const category = career.category || 'general';
            acc[category] = (acc[category] || 0) + 1;
            return acc;
        }, {});
        // Count by stress level
        const byStressLevel = allCareers.reduce((acc, career) => {
            const level = career.stressLevel || 'unknown';
            acc[level] = (acc[level] || 0) + 1;
            return acc;
        }, {});
        // Count by growth potential
        const byGrowthPotential = allCareers.reduce((acc, career) => {
