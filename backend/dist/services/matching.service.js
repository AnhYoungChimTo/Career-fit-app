...
function buildUserProfile(answers) {
    ...
    const getAnswer = (key) => answers[key]?.answer ?? answers[key];
    ...
    if (Array.isArray(energizing)) {
        if (energizing.includes('creating')) {
            profile.a2.creativity += 15;
            profile.a1.openness += 10;
        }
        if (energizing.includes('analyzing')) {
            profile.a2.analytical_thinking += 15;
        }
        if (energizing.includes('helping')) {
            profile.a1.agreeableness += 15;
            profile.a1.eq_emotional_intelligence += 10;
        }
        if (energizing.includes('organizing')) {
            profile.a1.conscientiousness += 15;
            profile.a2.time_management += 10;
        }
        if (energizing.includes('leading')) {
            profile.a1.extraversion += 15;
        }
        if (energizing.includes('learning')) {
            profile.a2.learning_agility += 15;
            profile.a2.willingness_to_learn += 10;
        }
        if (energizing.includes('communicating')) {
            profile.a2.communication_verbal += 10;
            profile.a2.communication_written += 10;
        }
        if (energizing.includes('building_relationships')) {
    ...
    if (Array.isArray(interests)) {
        if (interests.includes('data_patterns')) {
            profile.a2.analytical_thinking += 10;
        }
        if (interests.includes('creative_arts')) {
            profile.a2.creativity += 10;
        }
        if (interests.includes('people_society')) {
            profile.a1.eq_emotional_intelligence += 10;
            profile.a1.agreeableness += 5;
        }
        if (interests.includes('business_economics')) {
            profile.a2.analytical_thinking += 5;
        }
        if (interests.includes('innovation_trends')) {
            profile.a1.openness += 10;
        }
        if (interests.includes('science_research')) {
            profile.a2.analytical_thinking += 10;
            profile.a1.conscientiousness += 5;
        }
        if (interests.includes('social_issues')) {
            profile.a1.agreeableness += 10;
        }
        if (interests.includes('global_culture')) {
            profile.a1.openness += 10;
        }
    }
    ...
    if (Array.isArray(langs)) {
        if (langs.length >= 2)
            profile.a2.communication_written += 10;
        if (langs.length >= 3)
            profile.a2.communication_written += 5;
        if (langs.includes('english'))
            profile.a2.digital_literacy += 10;
    }
    ...
    if (Array.isArray(motivators)) {
        if (motivators.includes('solving_problems')) {
            profile.a2.analytical_thinking += 5;
        }
        if (motivators.includes('helping_others')) {
            profile.a3.impact_vs_money += 15;
            profile.a1.agreeableness += 5;
        }
        if (motivators.includes('creating_building')) {
            profile.a3.creativity_vs_structure += 15;
            profile.a2.creativity += 5;
        }
        if (motivators.includes('recognition_advancement')) {
            profile.a3.impact_vs_money -= 10;
        }
        if (motivators.includes('autonomy_independence')) {
            profile.a3.autonomy_vs_guidance += 15;
        }
        if (motivators.includes('teamwork_collaboration')) {
            profile.a3.collaboration_vs_solo += 10;
        }
        if (motivators.includes('innovation_change')) {
            profile.a1.openness += 10;
            profile.a3.creativity_vs_structure += 10;
        }
        if (motivators.includes('mastery_expertise')) {
            profile.a2.willingness_to_learn += 10;
        }
    }
    ...
    if (Array.isArray(valuePriorities)) {
        valuePriorities.forEach((val, idx) => {
            const weight = (6 - idx) * 5; // First item +30, last +5
            if (val === 'high_income')
                profile.a3.impact_vs_money -= weight / 2;
            if (val === 'learning_growth')
                profile.a3.stability_vs_growth += weight / 2;
            if (val === 'meaningful_impact')
                profile.a3.impact_vs_money += weight / 2;
            if (val === 'work_life_balance')
                profile.a3.work_life_balance += weight / 3;
            if (val === 'creative_freedom')
                profile.a3.creativity_vs_structure += weight / 2;
            if (val === 'job_security')
                profile.a3.stability_vs_growth -= weight / 2;
        });
    }
    ...
    if (challengePref === 'analytical_complex') {
        profile.a2.analytical_thinking += 10;
    }
    else if (challengePref === 'creative_innovative') {
        profile.a2.creativity += 10;
        profile.a3.creativity_vs_structure += 5;
    }
    else if (challengePref === 'people_relationships') {
        profile.a1.eq_emotional_intelligence += 10;
    }
    else if (challengePref === 'strategic_planning') {
        profile.a2.analytical_thinking += 5;
        profile.a1.openness += 5;
    }
    else if (challengePref === 'hands_on_practical') {
        profile.a2.technical_aptitude += 10;
    }
    else if (challengePref === 'large_scale_impact') {
        profile.a3.impact_vs_money += 10;
    }
    ...
    if (Array.isArray(workChars)) {
        if (workChars.includes('variety_diversity'))
            profile.a3.variety_vs_routine += 10;
        if (workChars.includes('innovation_cutting_edge'))
            profile.a1.openness += 5;
        if (workChars.includes('stability_predictability'))
            profile.a3.stability_vs_growth -= 10;
        if (workChars.includes('global_multicultural'))
            profile.a1.openness += 5;
        if (workChars.includes('social_impact'))
            profile.a3.impact_vs_money += 10;
        if (workChars.includes('flexibility_autonomy'))
            profile.a3.autonomy_vs_guidance += 10;
        if (workChars.includes('continuous_learning'))
            profile.a2.willingness_to_learn += 10;
    }
    ...
    if (Array.isArray(personalityTraits)) {
        if (personalityTraits.includes('analytical'))
            profile.a2.analytical_thinking += 10;
        if (personalityTraits.includes('creative'))
            profile.a2.creativity += 10;
        if (personalityTraits.includes('social'))
            profile.a1.extraversion += 10;
        if (personalityTraits.includes('organized'))
            profile.a1.conscientiousness += 10;
        if (personalityTraits.includes('adventurous'))
            profile.a1.openness += 10;
    }
    const teamRole = getAnswer('a1_team_role');
    if (teamRole === 'leader')
        profile.a1.extraversion += 10;
    else if (teamRole === 'analyzer')
        profile.a2.analytical_thinking += 10;
    else if (teamRole === 'mediator')
        profile.a1.agreeableness += 10;
    else if (teamRole === 'creative')
        profile.a2.creativity += 10;
 ...
 */
function calculateDimensionMatch(userDimension, careerDimension) {
    if (!careerDimension || Object.keys(careerDimension).length === 0)
        return 50;
    let totalScore = 0;
    let count = 0;
    for (const key of Object.keys(careerDimension)) {
        const required = careerDimension[key];
        const actual = userDimension[key];
 ...
 */
async function generateMatches(interviewId) {
    ...
    // Extract all answers
    const personalityData = interview.personalityData || {};
    const talentsData = interview.talentsData || {};
    const valuesData = interview.valuesData || {};
    ...
    const matches = await Promise.all(topMatches.map(async ({ career, fitScore }, index) => {
        console.log(`  ${index + 1}/${topMatches.length} - Generating for: ${career.name}`);
        const explanation = await generateCareerExplanation(career, allAnswers, userContext, fitScore, interview.interviewType);
 ...
 */
function calculateFitScore(userProfile, career, interviewType) {
 ...
 * Determine confidence level
 */
function getConfidenceLevel(interviewType, dataCompleteness) {
 ...
 */
async function generateCareerExplanation(career, answers, userContext, fitScore, interviewType) {
    // ═══════════════════════════════════════════════════════════════════
    ...
    try {
        ...
        const cached = career.cachedAnalysis;
...
- Be thorough — this will be the permanent reference for this career`;
            ...
            await prisma.career.update({
                where: { id: career.id },
                data: { cachedAnalysis: staticData },
            });
        ...
        const staticCache = career.cachedAnalysis;
 ...
 */
function buildUserContext(answers, profile) {
    const context = [];
    const getAnswer = (key) => answers[key]?.answer ?? answers[key];
    // Current situation
    const status = getAnswer('session_current_status');
    if (status)
        context.push(`Current Situation: ${status.replace(/_/g, ' ')}`);
    const field = getAnswer('session_current_field');
    if (field)
        context.push(`Current Field: ${field}`);
    const experience = getAnswer('session_experience_level');
    if (experience)
        context.push(`Experience: ${experience.replace(/_/g, ' ')}`);
    // Problem-solving approach
    const psa = getAnswer('a1_problem_solving_approach');
    if (psa)
        context.push(`Problem-Solving Approach: ${psa.replace(/_/g, ' ')}`);
    // Energizing activities
    const energizing = getAnswer('a1_energizing_activities');
    if (Array.isArray(energizing)) {
        context.push(`Activities That Energize Them: ${energizing.map((e) => e.replace(/_/g, ' ')).join(', ')}`);
    }
    // Interest areas
    const interests = getAnswer('a1_interest_areas');
    if (Array.isArray(interests)) {
        context.push(`Interest Areas: ${interests.map((i) => i.replace(/_/g, ' ')).join(', ')}`);
    }
    // Learning style
    const learnStyle = getAnswer('a2_learning_style');
    if (learnStyle)
        context.push(`Learning Style: ${learnStyle.replace(/_/g, ' ')}`);
    // Communication preference
    const commPref = getAnswer('a2_communication_preference');
    if (commPref)
        context.push(`Communication Preference: ${commPref.replace(/_/g, ' ')}`);
    // Quantitative comfort
    const quantComfort = getAnswer('a2_quantitative_comfort');
    if (quantComfort)
        context.push(`Comfort with Numbers/Data: ${quantComfort.replace(/_/g, ' ')}`);
    // Ambiguity tolerance
    const ambiguity = getAnswer('a1_ambiguity_tolerance');
    if (typeof ambiguity === 'number') {
    ...
    const valuePriorities = getAnswer('a3_value_priorities');
    if (Array.isArray(valuePriorities)) {
        context.push(`Value Priorities (ranked): ${valuePriorities.map((v) => v.replace(/_/g, ' ')).join(' > ')}`);
    }
    // Environment preference
    const envPref = getAnswer('a3_environment_preference');
    if (envPref)
        context.push(`Preferred Work Environment: ${envPref.replace(/_/g, ' ')}`);
    // Primary motivators
    const motivators = getAnswer('a3_primary_motivators');
    if (Array.isArray(motivators)) {
        context.push(`Primary Motivators: ${motivators.map((m) => m.replace(/_/g, ' ')).join(', ')}`);
    }
    // Collaboration style
    const collabStyle = getAnswer('a3_collaboration_style');
    if (collabStyle)
        context.push(`Collaboration Style: ${collabStyle.replace(/_/g, ' ')}`);
    // Challenge type
    const challengePref = getAnswer('a3_challenge_type_preference');
    if (challengePref)
        context.push(`Preferred Challenge Type: ${challengePref.replace(/_/g, ' ')}`);
    // Work-life balance
    const wlb = getAnswer('a3_work_life_balance_priority');
    if (typeof wlb === 'number') {
